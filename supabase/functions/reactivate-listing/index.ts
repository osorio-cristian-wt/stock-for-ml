...
// RF-47: reactivates publications the seller paused (sub_status
// paused_by_seller) after stock came back from 0. Out-of-stock pauses are NOT
// handled here — the regular stock push (PUT available_quantity > 0) already
// reactivates those automatically per ML docs. Body:
//   { ml_item_ids: ["MLA123", ...] }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body = await req.json().catch(() => ({})) as { ml_item_ids?: string[] };
    ...
    for (const itemId of ids) {
      try {
        ...
        await client.updateItemStatus(itemId, "active");
    ...
    return jsonResponse({ reactivated, errors });
  } catch (e) {
